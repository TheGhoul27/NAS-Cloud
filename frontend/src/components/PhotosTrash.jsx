...
import { ArrowLeft, Trash2, RotateCcw, AlertTriangle, FileImage, FileVideo, Clock, Calendar, X, CheckCircle, Camera } from 'lucide-react';
...
const PhotosTrash = () => {
  ...
  const fetchTrashItems = async () => {
    try {
      setLoading(true);
      const response = await filesAPI.listTrash('photos');
  ...
  const handleEmptyTrash = async () => {
    setProcessing(true);
    try {
      const response = await filesAPI.emptyTrash('photos');
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                  Items deleted from Photos • Auto-deleted after 30 days
                </p>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className={`text-lg font-medium ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                Photos trash is empty
              </p>
              <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'} mt-1`}>
                Items you delete from Photos will appear here
              </p>
            </div>
          ) : (
                ...
                <tbody>
                  {trashItems.map((item, index) => (
                    ...
                    >
                      <td className="p-4">
                        <div className="flex items-center">
                          {getFileType(item.original_name) === 'image' ? (
                            <FileImage className="h-5 w-5 text-green-500 mr-3 flex-shrink-0" />
                          ) : getFileType(item.original_name) === 'video' ? (
                            <FileVideo className="h-5 w-5 text-red-500 mr-3 flex-shrink-0" />
                          ) : (
                            <Camera className="h-5 w-5 text-gray-500 mr-3 flex-shrink-0" />
                          )}
          ...
          <div className={`w-full max-w-md rounded-lg shadow-xl ${
            ...
            <div className="p-6">
              ...
              <p className={`mb-6 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                Are you sure you want to restore "{itemToRestore.original_name}" to your Photos?
              </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={`w-full max-w-md rounded-lg shadow-xl ${
            ...
            <div className="p-6">
              ...
              <p className={`mb-6 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                Are you sure you want to permanently delete all {trashItems.length} items in the Photos trash? This action cannot be undone.
              </p>
